...
export class EntryItemService {
  ...
  createEntryItemFormGroup() {
    return this.formBuilder.group({
      categoryId: this.formBuilder.control<number | undefined>(
        {value: undefined, disabled: false},
        Validators.required,
      ),
      amount: this.formBuilder.control<number | undefined>(
        {value: undefined, disabled: false},
        [Validators.required, this.validateAmountNotZero.bind(this)],
      ),
  ...
  validateAmountNotZero(control: AbstractControl): ValidationErrors {
    const amount =
      typeof control.value === 'number'
        ? control.value
        : this.parseNumberService.parse(control.value);
